...
public struct NIOSSHAvailableUserAuthenticationMethods: OptionSet {
    ...
    /// RFC 4256. How most servers ask for a one-time code.
...
public extension NIOSSHUserAuthenticationOffer {
    enum Offer {
        ...
        case password(Password)
        case hostBased(HostBased)
        /// RFC 4256. Offering this starts a challenge/response exchange: the
        /// server sends one or more `SSH_MSG_USERAUTH_INFO_REQUEST`s, each
        /// answered through
...
public extension NIOSSHUserAuthenticationOffer.Offer {
    ...
    struct KeyboardInteractive {
        /// A comma-separated hint at which submethod to use, such as `"pam"`.
        /// Almost always empty, which lets the server choose.
