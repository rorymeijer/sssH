...
extension SSHMessage {
    ...
    struct UserAuthRequestMessage: Equatable {
        ...
        enum Method: Equatable {
            ...
            /// RFC 4256. The payload is the (usually empty) submethod hint.
    ...
    /// RFC 4256 §3.2.
    ///
    /// Note the message ID: 60, the same as ``UserAuthPKOKMessage``. SSH reuses
    /// the IDs 60–79 per authentication method, so which one a byte means
    /// depends on the method in flight — see `ByteBuffer.readSSHMessage(methodInFlight:)`.
    struct UserAuthInfoRequestMessage: Equatable {
        ...
        struct Prompt: Equatable {
            var prompt: String
            /// `false` for a password-like prompt, which a client must not echo.
            var echo: Bool
        }
        ...
        var languageTag: String
        /// May legitimately be empty: a server can use this message purely to
        /// display text, and that still requires an immediate empty response.
        var prompts: [Prompt]
    }
    ...
    struct UserAuthInfoResponseMessage: Equatable {
        ...
        /// Must contain exactly one entry per prompt in the request it answers.
...
extension ByteBuffer {
    ...
    /// it will return nil.
    /// - Parameter methodInFlight: which authentication method's
    ///   method-specific message IDs (60–79) are currently meaningful. SSH
    ///   reuses those IDs per method, so a stateless parser cannot resolve
    ///   them: ID 60 is `SSH_MSG_USERAUTH_PK_OK` during a public-key exchange
    ///   and `SSH_MSG_USERAUTH_INFO_REQUEST` during a keyboard-interactive one.
    mutating func readSSHMessage(
        methodInFlight: SSHUserAuthMethodInFlight = .none
    ) throws -> SSHMessage? {
        try self.rewindOnNilOrError { `self` in
            ...
            case SSHMessage.UserAuthPKOKMessage.id:
                // 60 means two different things. Without knowing which method
                // is in flight, parsing this is a coin toss.
                switch methodInFlight {
    ...
    mutating func readUserAuthInfoRequestMessage() -> SSHMessage.UserAuthInfoRequestMessage? {
        self.rewindOnNilOrError { `self` in
            ...
            // Each prompt costs at least five bytes on the wire (a four-byte
            // length plus a boolean), so a count larger than the remaining
            // bytes can only be a corrupt or hostile message. Checking before
            // reserving capacity is what stops it becoming an allocation.
            guard promptCount <= UInt32(self.readableBytes / 5) else {
                return nil
            }
    ...
    mutating func readUserAuthInfoResponseMessage() -> SSHMessage.UserAuthInfoResponseMessage? {
        self.rewindOnNilOrError { `self` in
            guard let responseCount = self.readInteger(as: UInt32.self) else {
                return nil
            }
            guard responseCount <= UInt32(self.readableBytes / 4) else {
                return nil
            }
    ...
    mutating func readUserAuthRequestMessage() throws -> SSHMessage.UserAuthRequestMessage? {
        try self.rewindOnNilOrError { `self` in
            ...
            case "keyboard-interactive":
                // RFC 4256 §3.1: a deprecated language tag, then the submethod
                // hint. Both are usually empty.
                guard
            ...
            case "publickey":
                ...
                if NIOSSHPublicKey.knownAlgorithms.contains(where: { $0.elementsEqual(algorithmName.readableBytesView) }) {
                    ...
                    // RFC 8332: the negotiated algorithm name and the name
                    // inside the key blob differ for RSA — "rsa-sha2-512" over
                    // an "ssh-rsa" blob — so either is a legitimate match.
                    let algorithmBytes = algorithmName.readableBytesView
                    guard algorithmBytes.elementsEqual(publicKey.keyPrefix)
                        || algorithmBytes.elementsEqual(publicKey.keyBlobPrefix)
                    else {
...
extension ByteBuffer {
    ...
    mutating func writeUserAuthRequestMessage(_ message: SSHMessage.UserAuthRequestMessage) -> Int {
        ...
        case .keyboardInteractive(let submethods):
            ...
            // The language tag is deprecated by RFC 4256 and must be empty.
