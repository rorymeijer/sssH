...
public protocol NIOSSHPublicKeyProtocol {
    ...
    ///
    /// This is the name that is *negotiated*: the host key algorithm, and the
    /// algorithm named in a public-key authentication request.
    static var publicKeyPrefix: String { get }

    /// The identifier written inside the key blob itself.
    ///
    /// Usually the same as ``publicKeyPrefix``, and defaulted to it. RFC 8332
    /// is the exception that makes this necessary: an RSA key blob always
    /// begins with `"ssh-rsa"` even when the negotiated algorithm is
    /// `"rsa-sha2-256"` or `"rsa-sha2-512"`. Writing the negotiated name into
    /// the blob produces something that looks plausible and that OpenSSH
    /// rejects.
    static var keyBlobPrefix: String { get }
...
public extension NIOSSHPublicKeyProtocol {
    /// Defaults to ``publicKeyPrefix``, which is correct for every algorithm
    /// except the RFC 8332 RSA ones.
    static var keyBlobPrefix: String {
