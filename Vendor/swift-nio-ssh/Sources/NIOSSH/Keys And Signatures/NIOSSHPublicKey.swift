...
extension NIOSSHPublicKey {
    ...
    /// The identifier inside this key's wire blob, which RFC 8332 separates
    /// from the negotiated algorithm name for RSA. Identical to ``keyPrefix``
    /// for everything else.
    internal var keyBlobPrefix: String.UTF8View {
        switch self.backingKey {
        case .custom(let publicKey):
            return publicKey.keyBlobPrefix.utf8
...
extension ByteBuffer {
    ...
    mutating func writeSSHHostKey(_ key: NIOSSHPublicKey) -> Int {
        ...
        case .custom(let key):
            writtenBytes += writeSSHString(key.keyBlobPrefix.utf8)
    ...
    mutating func writePublicKeyWithoutPrefix(_ key: NIOSSHPublicKey) -> Int {
        ...
        case .ecdsaP521(let key):
            return self.writeECDSAP521PublicKey(baseKey: key)
        case .custom(let key):
            var writtenBytes = writeSSHString(key.keyBlobPrefix.utf8)
            writtenBytes += key.write(to: &self)
            return writtenBytes
        case .certified:
        ...
        try self.rewindOnNilOrError { buffer in
            if keyIdentifierBytes.elementsEqual(NIOSSHPublicKey.ed25519PublicKeyPrefix) {
                ...
                for type in NIOSSHPublicKey.customPublicKeyAlgorithms {
                    // Matched on the blob prefix, which is what is actually on
                    // the wire. Several negotiated algorithms may share one —
                    // the three RFC 8332 RSA names all use "ssh-rsa" — and the
                    // key material is identical, so the first match is right.
                    if keyIdentifierBytes.elementsEqual(type.keyBlobPrefix.utf8) {
