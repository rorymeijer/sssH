...
struct SSHPacketParser {
    ...
    ///
    /// The one piece of state this parser has, and it exists because SSH's
    /// message IDs are not self-describing: ID 60 is `SSH_MSG_USERAUTH_PK_OK`
    /// during a public-key exchange and `SSH_MSG_USERAUTH_INFO_REQUEST` during
    /// a keyboard-interactive one. The user-authentication state machine keeps
    /// it current.
    internal var userAuthMethodInFlight: SSHUserAuthMethodInFlight = .none
    ...
    private mutating func parsePlaintext(length: UInt32) throws -> SSHMessage? {
        try self.buffer.rewindReaderOnError { buffer in
            ...
            guard let message = try content.readSSHMessage(methodInFlight: self.userAuthMethodInFlight),
